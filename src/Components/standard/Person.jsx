...
function Person(props) {
    ...
    useEffect(() => {
        const qPers = query(collection(db, "users"), where("uid", "==", uid))
        const getPersonInfo = async () => {
            const data = await getDocs(qPers)
            setUsers(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })))
            setRankShort(users.map((user) => user.rank))
        }
        const qRank = query(collection(db, "ranks"), where("short", "==", "dc"))
        const getRankInfo = async () => {
            const data = await getDocs(qRank)
            setRank(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })))
        }
    ...
    return (
        <div>
            ...
            {users.map((user) => {
                return (
                    <div className='main ml-28 mt-10 flex flex-col'>
                        <p className='text-3xl font-extrabold mb-5'>{user.rank.toUpperCase()}, {user.firstname} {user.name}</p>
            ...
            {rank.map((rank) => {
                return (
                    <div className="ml-28 flex flex-col mr-5">
                        <div className="flex">
                            <h1 className="text-1xl font-bold p-2 border-b-orange-500 border-t-0 border-x-0 border text-left flex-grow">Aufgaben</h1>
                            <button className="text-1xl font-bold p-2 border-b-orange-500 border-t-0 border-x-0 border text-right" onClick={() => toggle()}>{tasksToggle? "-" : "+"}</button>
                        </div>
                        <div className={`collapsible ${tasksToggle? "on" : "off"}`}>
